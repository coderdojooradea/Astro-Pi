if __name__ == "__main__":
    # -*- coding: utf-8 -*-
        ...
        cam = PiCamera()
        cam.resolution = (4056, 3040) #(2028,1520) #(1920,1080)
        ...
        runningTime = 9*30 #180*60 seconds
        photodelay = 3
        ph_counter = 1
        ...
        while currentTime <= startTime + datetime.timedelta(seconds=runningTime):
            try:
                ...
                if currentTime > lastPhotoTime + datetime.timedelta(seconds = photodelay):# and (sunup or sunup30):
                    ...
                    photo_name = imgFolder + "/CDOSR_{:%H%M%S}_{:04d}.jpg".format(currentTime, ph_counter) 
                    cam.capture(photo_name)
                    if ph_counter>1:
                        get_time_difference(last_photo_name, photo_name)
                    lastPhotoTime = currentTime
                    ph_counter += 1
                    
                    if ph_counter>1:
                        image_1_cv, image_2_cv = convert_to_cv(last_photo_name, photo_name) # Create OpenCV image objects
